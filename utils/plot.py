...
def text_to_pil(t, imgsize, alphabet, font, w=128, h=128, linewidth=8):
    ...
    # Split into full attribute phrases (by comma)
    attributes = [attr.strip() for attr in text_sample.split(",") if attr.strip()]

    y_text = h
    # num_lines = len(lines);
    num_lines = len(attributes)
    # for l, line in enumerate(lines):
    for l, attr in enumerate(attributes):
        # width, height = font.getsize(line)
        # bbox = font.getbbox(line)
        bbox = font.getbbox(attr)
        width, height = bbox[2] - bbox[0], bbox[3] - bbox[1]

        # draw.text((0, (h/2) - (num_lines/2 - l)*height), line, (0, 0, 0), font=font)
        draw.text((0, (h/2) - (num_lines/2 - l)*height), attr, (0, 0, 0), font=font)
        y_text += height
    if imgsize[0] == 3:
