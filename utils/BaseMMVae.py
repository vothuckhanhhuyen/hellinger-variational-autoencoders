...
from divergence_measures.mm_div import poe, helvae
...
class BaseMMVae(ABC, nn.Module):
    ...
    def set_fusion_functions(self):
        ...
        if self.flags.modality_moe:
            ...
            self.calc_joint_divergence = self.divergence_static_prior;
        elif self.flags.modality_helvae:
            self.modality_fusion = self.helvae_fusion;
            self.fusion_condition = self.fusion_condition_poe;
            self.calc_joint_divergence = self.divergence_static_prior;
        elif self.flags.joint_helvae:
            self.modality_fusion = self.helvae_fusion;
            self.fusion_condition = self.fusion_condition_joint;
            self.calc_joint_divergence = self.divergence_static_prior;
    ...
    def helvae_fusion(self, mus, logvars, ver_type="f32", weights=None):
        mu_helvae, logvar_helvae = helvae(mus, logvars, ver_type);
        return [mu_helvae, logvar_helvae];
        ...
        for k, s_key in enumerate(self.subsets.keys()):
            if s_key != '':
                ...
                if mods_avail:
                    ...
                    if self.flags.modality_helvae or self.flags.joint_helvae:
                        s_mu, s_logvar = self.modality_fusion(mus_subset,
                                                            logvars_subset,
                                                            ver_type=self.flags.ver_type,
                    ...
                    if self.fusion_condition(mods, input_batch):
                        ...
                        included_keys.append(s_key)
                        included_len_subset_keys.append(len(mods))
